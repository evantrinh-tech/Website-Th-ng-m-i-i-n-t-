...
(function () {
    ...
    const VARIANTS_API = './backend/api/product-variants.php';
    ...
    async function doAddToCart(variantId, qty, btn) {
        const token = localStorage.getItem('kx_auth_token');
        if (!token) {
            if (window.showToast) window.showToast('Vui lòng đăng nhập để thêm vào giỏ hàng.', 'warning');
        ...
        btn.disabled = true;
        btn.innerHTML = '<span class="material-symbols-outlined text-[18px]">hourglass_top</span>';
        ...
        try {
            ...
            if (data.success) {
                btn.innerHTML = '<span class="material-symbols-outlined text-[18px]">check_circle</span> Đã thêm';
                ...
                if (window.showToast) window.showToast(data.message || 'Không thể thêm vào giỏ hàng.', 'error');
                btn.innerHTML = origHTML;
                btn.disabled = false;
            }
        } catch (err) {
    ...
    async function fetchVariantId(slug) {
        try {
            const res  = await fetch(VARIANTS_API + '?slug=' + encodeURIComponent(slug));
            const data = await res.json();
            if (data.success && data.first_id) return data.first_id;
            console.warn('[add-to-cart] Không tìm thấy variant, slug =', slug, '| response:', data);
        } catch (e) {
    ...
    async function handleClick(e) {
        ...
        const slug = btn.dataset.productSlug
            || btn.closest('[data-slug]')?.dataset.slug
            || '';

        if (!slug) {
            if (window.showToast) window.showToast('Vui lòng xem chi tiết sản phẩm để thêm vào giỏ hàng.', 'info');
            return;
        }
        ...
        if (vid) {
            ...
            if (window.showToast) window.showToast('Sản phẩm chưa có trong kho dữ liệu.', 'warning');
            btn.innerHTML = origHTML;
            btn.disabled = false;
        }
    }
    ...
    function bindButtons() {
        document.querySelectorAll('[data-cart-action="add-to-cart"]').forEach(btn => {
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        if (grid) {
            new MutationObserver(bindButtons).observe(grid, { childList: true, subtree: true });
        }
    });
