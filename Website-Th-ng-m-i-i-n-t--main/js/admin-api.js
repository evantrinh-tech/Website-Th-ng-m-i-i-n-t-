...
async function loadAdminStats() {
    ...
    try {
        ...
        if (!data.success) {
            if (res.status === 403) {
                window.showToast('Bạn không có quyền truy cập trang quản trị.', 'error');
        ...
        if (ordersEl)   ordersEl.textContent   = d.total_orders?.toLocaleString('vi-VN') || '0';
        if (usersEl)    usersEl.textContent    = d.total_users?.toLocaleString('vi-VN')  || '0';
        if (productsEl) productsEl.textContent = d.total_products?.toLocaleString('vi-VN') || '0';
...
async function loadRecentOrders() {
    ...
    tbody.innerHTML = '<tr><td colspan="5" class="px-8 py-5 text-center text-on-surface-variant text-sm">Đang tải...</td></tr>';
    ...
    try {
        ...
        if (!data.success || !data.data.length) {
            tbody.innerHTML = '<tr><td colspan="5" class="px-8 py-5 text-center text-on-surface-variant text-sm">Chưa có đơn hàng nào.</td></tr>';
            return;
        }

        tbody.innerHTML = data.data.slice(0, 10).map(order => {
        ...
        tbody.innerHTML = '<tr><td colspan="5" class="px-8 py-5 text-center text-error text-sm">Lỗi tải dữ liệu.</td></tr>';
    }
}
