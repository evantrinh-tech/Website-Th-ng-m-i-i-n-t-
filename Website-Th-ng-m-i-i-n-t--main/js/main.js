...
function animateAddToCartButton(button) {
  ...
  const isIconOnly =
    iconText === "add_shopping_cart" &&
    !(button.innerText || "").includes("Thêm vào giỏ");

  if (isIconOnly) {
    button.innerHTML = '<span class="material-symbols-outlined">check</span>';
  } else {
    button.innerHTML =
      '<span class="material-symbols-outlined text-[20px]">check_circle</span> Đã thêm';
...
function ensureCartBadges() {
  document.querySelectorAll("header, nav").forEach((root) => {
    const cartTargets = Array.from(root.querySelectorAll("a, button")).filter(
      (el) => {
        const icon =
          el.querySelector(".material-symbols-outlined") ||
          (el.classList.contains("material-symbols-outlined") ? el : null);
    ...
    cartTargets.forEach((target) => {
      ...
      badge.className =
        "absolute -top-2 -right-2 bg-secondary text-on-secondary text-[10px] font-bold min-w-[16px] h-4 px-1 rounded-full flex items-center justify-center pointer-events-none";
      badge.innerText = "0";
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (urlPath.includes("cart.html")) {
    const updatePrice = (qty) => {
      ...
      const lineTotalPrices = document.querySelectorAll(
        ".text-xl.font-extrabold.text-primary",
      );
      if (lineTotalPrices.length > 0) {
        lineTotalPrices[0].innerText =
          (unitPrice * qty).toLocaleString("vi-VN") + "₫";
      }
      ...
      if (summaryValues.length >= 2) {
        ...
        const finalTotal = document.querySelector(
          ".font-headline.font-extrabold.text-3xl",
        );
        if (finalTotal) {
          finalTotal.innerText =
            (subTotal + taxTotal).toLocaleString("vi-VN") + "₫";
        }
      }
    };
      ...
      deleteBtn.addEventListener("click", (e) => {
        ...
        if (itemRow) {
          ...
          setTimeout(() => {
            ...
            if (headerMsg)
              headerMsg.innerText = "Bạn chưa có sản phẩm nào trong giỏ hàng";
          }, 300);
        }
      });
    }
    ...
    // Mã giảm giá
    const applyPromoBtn = document.querySelector(
      "button.bg-primary.text-on-primary.px-4.py-3",
    );
    if (applyPromoBtn) {
      applyPromoBtn.addEventListener("click", () => {
        ...
        if (input && input.value.trim() !== "") {
          ...
          setTimeout(
            () => alert("Đã áp dụng mã giảm giá: " + input.value),
            100,
          );
        } else {
  ...
  //     document.getElementById("checkout-complete-btn") ||
  //     Array.from(document.querySelectorAll("button")).find((btn) =>
  //       btn.innerText.includes("HOÀN TẤT ĐẶT HÀNG"),
  //     );
  ...
  //         '<span class="material-symbols-outlined">check_circle</span> ĐẶT HÀNG THÀNH CÔNG';
  ...
  //         (item) => !(item.selected !== false && paidNames.has(item.name)),
  //       );
  ...
  if (urlPath.includes("product-detail.html")) {
    ...
    const thumbnailsContainer = document.querySelectorAll(
      ".col-span-2 .aspect-square",
    );
    const mainImage = document.querySelector(
      ".col-span-10 .aspect-\\[4\\/5\\] img",
    );
    ...
    if (colorBtns.length > 0) {
      ...
      colorBtns.forEach((btn, index) => {
        btn.addEventListener("click", () => {
          ...
          const stickyLabel = document.querySelector(
            ".fixed.bottom-0 p.text-xs.text-outline",
          );
          if (stickyLabel) {
            const currentLens =
              stickyLabel.innerText.split("/")[1]?.trim() || "Tròng mẫu";
    ...
    const lensBtns = document.querySelectorAll(
    ...
    const tabs = document.querySelectorAll(
      ".flex.border-b.border-surface-container-highest button",
    );
    if (tabs.length > 0) {
      tabs.forEach((tab) => {
    ...
    const uploadBtn = document.querySelector(
      ".p-6.bg-surface-container-low button",
    );
    if (uploadBtn) {
  ...
  if (urlPath.includes("products.html") || urlPath.includes("index.html")) {
    ...
    const pageBtns = document.querySelectorAll(
      ".mt-20.flex.justify-center.items-center button",
    );
    ...
    const clearAllBtn = document.querySelector(
      "button.text-xs.font-semibold.text-primary.underline",
    );
    if (clearAllBtn) {
    ...
    const filterCheckboxes = document.querySelectorAll(
      'aside input[type="checkbox"], #mobile-filter-drawer input[type="checkbox"]',
    );
    ...
    priceSliders.forEach((slider) => {
      ...
      if (container) {
        ...
        valueDisplay = container.querySelectorAll(
          ".flex.justify-between.text-\\[11px\\] span",
        )[1];
      }
    ...
    const genderContainers = document.querySelectorAll(
      ".space-y-3 .flex.flex-wrap.gap-2",
    );
    genderContainers.forEach((container) => {
      ...
      btns.forEach((btn) => {
        btn.addEventListener("click", (e) => {
          ...
          btns.forEach((b) => {
            b.className =
              "px-3 py-1.5 rounded-lg text-xs text-slate-500 bg-surface-container-low hover:bg-surface-container transition-colors";
          });
          // Set active
          btn.className =
            "px-3 py-1.5 rounded-lg text-xs font-semibold bg-white text-[#002366] ring-1 ring-outline-variant shadow-sm transition-colors";
  ...
  if (
    urlPath.includes("index.html") ||
    urlPath === "/" ||
    urlPath === "" ||
    urlPath.endsWith("\\")
  ) {
    const timerBlocks = document.querySelectorAll(
      ".bg-secondary.text-on-secondary.px-3.py-2.rounded-lg.font-bold",
    );
    ...
    document.querySelectorAll("button").forEach((btn) => {
      if (
        btn.innerText.includes("Thử ngay miễn phí") ||
        btn.innerText.includes("Virtual Try-On") ||
        btn.innerText.includes("Xem Bộ Sưu Tập")
      ) {
        btn.addEventListener("click", (e) => {
          ...
          if (!btn.getAttribute("onclick")) {
            e.preventDefault();
            showToast(
              "Đang kết nối khối Camera để quét AR khuôn mặt...",
              "info",
            );
          }
        });
    ...
    const rankingBtn = Array.from(document.querySelectorAll("button")).find(
      (btn) => btn.innerText.includes("Xem bảng xếp hạng"),
    );
    ...
    if (rankingBtn) {
      rankingBtn.addEventListener("click", (e) => {
        ...
        modal.className =
          "fixed inset-0 z-[1000] flex items-center justify-center bg-black/50 backdrop-blur-md opacity-0 transition-opacity duration-300 pointer-events-none p-4";
        ...
        setTimeout(() => {
          ...
          document
            .getElementById("ranking-content")
  ...
  links.forEach((link) => {
    link.addEventListener("click", (e) => {
      ...
      if (
        !targetAttr ||
        targetAttr === "#" ||
        targetAttr.startsWith("javascript:")
      )
        return;
      ...
      try {
        ...
        if (targetUrl.pathname === currentUrl.pathname) {
          if (targetUrl.hash) {
            ...
            if (window.showToast)
              window.showToast("Bạn đang xem bảng này rồi! 👇", "info");
            return;
          }
        }
      } catch (e) {}
...
document.addEventListener("click", (e) => {
  ...
  if (true) {
    const productBlock =
      ...
      btn.closest(".bg-surface-container-low") ||
      btn.closest('div[class*="rounded"]');
    let productName = "Cặp Kính Kính Xanh";
    let productImg =
      "https://lh3.googleusercontent.com/aida-public/AB6AXuADIZl6BGMy5DdsXak_XA0rcOxwIsYHt9dv7TvpgXDxIbLTIT_h-4jkkj2aN2q8mIYj2vpV7t9isFzQI7utEgBjbjM2YPNlF5JvpaWxIZTXtmjLYAEHIkNG99mn_lAuGkq1s3_JxyelTLJ2rpLQD1aWKKJcETIv2Dp9dVGP_rGVF3PPk3YZStXMFv8Qaj4MstQ0oDQqBf_NP6FHhjYXE_RrISqjmTWDqNUYMkpu5yTa17ZSh0cHo7prLh41QFAL16bTu3gTXZ7iKqI";
    let productPrice = 1200000;

    if (productBlock && productBlock.tagName !== "BODY") {
      const titleEl =
        productBlock.querySelector("h3") || productBlock.querySelector("h4");
      ...
      const priceEls = Array.from(
        productBlock.querySelectorAll("span, p, div"),
      ).filter(
        (el) =>
          (el.innerText || "").includes("đ") ||
          (el.innerText || "").includes("₫"),
      );
      if (priceEls.length > 0) {
        const priceText = priceEls[0].innerText
          .split("đ")[0]
          .split("₫")[0]
          .replace(/[^0-9]/g, "");
...
function renderCart() {
  ...
  let allSelected =
    globalCart.length > 0 && globalCart.every((i) => i.selected !== false);
  ...
  if (globalCart.length === 0) {
    html =
      '<div class="text-center py-16 bg-surface-container-low rounded-2xl border-dashed border-2 border-outline-variant/50"><span class="material-symbols-outlined block text-6xl mb-4 text-slate-300">shopping_basket</span><p class="font-bold text-lg text-primary">Giỏ hàng rỗng!</p><a href="products.html" class="inline-block mt-4 text-secondary font-bold hover:underline">Khám phá sản phẩm</a></div>';
  }

  container.innerHTML = html;

  if (headerPar)
    headerPar.innerText = `Bạn có ${totalQty} sản phẩm trong giỏ hàng`;
  if (tamTinhEl) tamTinhEl.innerText = formatCurrency(totalPrice);
  if (vatEl) vatEl.innerText = formatCurrency(totalPrice * 0.08);
  if (thanhTienEl)
    thanhTienEl.innerText = formatCurrency(totalPrice + totalPrice * 0.08);

  const checkoutBtn = document.querySelector(
    'a[href="checkout.html"], button[data-checkout-btn]',
  );
  if (checkoutBtn) {
...
headerLinks.forEach((link) => {
  ...
  const isCurrentPage =
    normalizedHref &&
    urlPath.includes(normalizedHref) &&
    href !== "index.html#flash-sale";
...
// ============================================
const searchInputs = document.querySelectorAll(
  'input[data-product-search="true"]',
);
searchInputs.forEach((input) => {
  input.addEventListener("input", (e) => {
    ...
    if (window.location.pathname.includes("products.html")) {
      ...
      const productCards = document.querySelectorAll(
        ".grid.grid-cols-2.md\\:grid-cols-3 > div, .grid.grid-cols-1.md\\:grid-cols-3 > div",
      );
...
if (window.location.pathname.includes("products.html")) {
  ...
  if (productGrid) {
    ...
    const priceLabel = sidebar?.querySelector(".flex.justify-between")
      ?.children[1];
    ...
    const getProductPrice = (el) => {
      ...
      const priceText =
        el.querySelector(".text-lg.font-bold.text-primary")?.innerText || "0";
      return parseInt(priceText.replace(/[^0-9]/g, "")) || 0;
    };

    const updateResultCount = () => {
      const visibleCount = Array.from(productGrid.children).filter(
        (p) =>
          p.style.display !== "none" && !p.classList.contains("animate-pulse"),
      ).length;
    ...
    const performFiltering = () => {
      const maxPriceValue = priceSlider
      ...
      const selectedLens = Array.from(
        document.querySelectorAll("input[data-lens]:checked"),
      ).map((cb) => cb.getAttribute("data-lens"));

      const activeGenderBtn = document.querySelector(
        "button[data-gender].active-filter",
      );
      const activeGender = activeGenderBtn
        ? activeGenderBtn.getAttribute("data-gender")
        : null;

      const activeMatItem = document.querySelector(
        "[data-material].active-filter",
      );
      const activeMaterial = activeMatItem
        ? activeMatItem.getAttribute("data-material")
        : null;
    ...
    document
      .querySelectorAll(
        'input[type="checkbox"][data-brand], input[type="checkbox"][data-lens], input[type="range"]',
      )
      .forEach((el) => {
    ...
    document.querySelectorAll("button[data-gender]").forEach((btn) => {
      btn.addEventListener("click", () => {
        ...
        if (!isAlreadyActive) {
          ...
          document
            .querySelectorAll(`button[data-gender="${gender}"]`)
            .forEach((target) => {
    ...
    document.querySelectorAll("[data-material]").forEach((item) => {
      item.addEventListener("click", () => {
        ...
        if (!isAlreadyActive) {
          document
            .querySelectorAll(`[data-material="${material}"]`)
            .forEach((target) => {
              target.classList.add(
                "active-filter",
                "font-bold",
    ...
    if (sortSelect) {
      sortSelect.addEventListener("change", (e) => {
        ...
        if (window.showToast)
          window.showToast(`Đã sắp xếp: ${sortType}`, "success");
      });
    }
    performFiltering();
  }
...
(function () {
  ...
  function parsePriceText(text) {
    ...
    for (const part of parts) {
      ...
      if (cleaned.length >= 4) {
        // Heuristic: prices are typically >= 1000₫
  ...
  function firstPriceInBlock(block) {
    ...
    const candidates = Array.from(
      block.querySelectorAll("span, p, div, b, strong, font"),
    );
    for (const node of candidates) {
      ...
      const hasChildrenCandidates =
        Array.from(node.querySelectorAll("span, p, b, strong, font")).length >
        0;
  ...
  function extractProductData(trigger, overrides = {}) {
    const block =
      trigger?.closest(
        "[data-price], .group, .bg-surface-container-lowest, .bg-surface-container-low, .rounded-xl, .rounded-2xl, .rounded-3xl",
      ) || document.body;
    const titleEl = block.querySelector(
      "h1, h2, h3, h4, .font-headline.font-bold, .font-headline.font-extrabold",
    );
    const imageEl = block.querySelector("img");
    let name =
      overrides.name ||
      (titleEl ? titleEl.textContent.trim() : "Sản phẩm Kính Xanh");
    let img =
      overrides.img || (imageEl ? imageEl.getAttribute("src") || "" : "");
    let price =
      Number(overrides.price) ||
      Number(block.getAttribute("data-price")) ||
      firstPriceInBlock(block) ||
      0;

    if (isProductDetailPage) {
      const title = document.querySelector("h1");
      const mainImage =
        document.querySelector(".col-span-10 img") ||
        document.querySelector('[data-detail="main_image"]');
      if (title) name = title.textContent.trim();
      if (mainImage) img = mainImage.getAttribute("src") || img;
      const pagePriceEl = Array.from(document.querySelectorAll("span")).find(
        (el) =>
          /₫|đ/.test(el.textContent || "") &&
          !(window.getComputedStyle(el).textDecorationLine || "").includes(
            "line-through",
          ),
      );
  ...
  function bindAddToCartButtonsFinal() {
    document
      .querySelectorAll(
        '[data-action="add-to-cart"], [data-cart-action="add-to-cart"]',
      )
      .forEach((originalBtn) => {
  ...
  function renderCartFinal() {
    ...
    if (headerPar) {
      ...
      headerPar.textContent =
        overallQty > 0
          ? `Bạn có ${overallQty} sản phẩm trong giỏ hàng`
    ...
    if (selectAll) {
      selectAll.addEventListener("change", () => {
        const next = loadCartState().map((item) => ({
          ...item,
  ...
  function renderCheckoutFinal() {
    ...
    if (
      document.getElementById("checkout-items-container")?.dataset.managed ===
      "api"
    )
      return;
  ...
  //       return;
  //     }
  //     const remaining = loadCartState().filter(
    ...
    if (isSupportPage) {
      ...
      if (form) {
        form.addEventListener("submit", (e) => {
          e.preventDefault();
          showToast(
            "Gửi yêu cầu thành công! Bộ phận hỗ trợ sẽ phản hồi sớm.",
            "success",
          );
          form.reset();
        });
  ...
  function bindProductFilteringAndSortingFinal() {
    ...
    const sortSelect = document.querySelector("select");
    const priceSlider = document.querySelector('aside input[type="range"]');
    const priceDisplays = Array.from(
      document.querySelectorAll("[data-price-range-label] span"),
    ).slice(-1);
    const resultCount = document.getElementById("filter-result-count");

    function getRating(card) {
      const ratingText = Array.from(card.querySelectorAll("span, p"))
    ...
    function applyFiltersAndSort() {
      ...
      const selectedBrands = Array.from(
        document.querySelectorAll("input[data-brand]:checked"),
      ).map((i) => i.dataset.brand);
      const selectedLens = Array.from(
        document.querySelectorAll("input[data-lens]:checked"),
      ).map((i) => i.dataset.lens);
      const activeGenderBtn = document.querySelector(
        "[data-gender].active-filter",
      );
      const activeMaterial = document.querySelector(
        "[data-material].active-filter",
      );
      ...
      const processed = cards
        .map((card) => {
          ...
          if (
            selectedLens.length &&
            !selectedLens.some((l) => lensList.includes(l))
          )
            visible = false;
          if (
            activeGender &&
            !["unisex", gender].includes(activeGender) &&
            gender !== activeGender
          )
            visible = false;
          if (activeMaterialValue && material !== activeMaterialValue)
            visible = false;
      ...
      processed.sort((a, b) => {
        if (sortType.includes("Giá thấp"))
          return Number(a.dataset.price) - Number(b.dataset.price);
        if (sortType.includes("Giá cao"))
    ...
    document
      .querySelectorAll("input[data-brand], input[data-lens]")
    ...
    document.querySelectorAll("[data-gender]").forEach((btn) =>
      btn.addEventListener("click", () => {
        ...
        if (!already) {
          document
            .querySelectorAll(`[data-gender="${sameValue}"]`)
            .forEach((item) => {
              item.classList.add(
                "active-filter",
                "bg-white",
    ...
    document.querySelectorAll("[data-material]").forEach((item) =>
      item.addEventListener("click", () => {
        ...
        if (!already) {
          document
            .querySelectorAll(`[data-material="${value}"]`)
            .forEach((el) => {
  ...
  function initMobileSidebarAdvanced() {
      ...
      if (cartLink && cartLink.parentElement) {
        ...
        if (rightControls) {
          ...
          hamburgerBtn.className =
            "mobile-menu-btn lg:hidden xl:hidden p-2 transform scale-95 hover:bg-surface-container transition-colors flex items-center justify-center rounded-full text-primary";
    ...
    header.addEventListener("click", (e) => {
      ...
      const icon =
        btn.querySelector(".material-symbols-outlined") ||
        (btn.classList.contains("material-symbols-outlined") ? btn : null);
      if (icon && icon.textContent.trim() === "menu") {
        openMenu(e);
      }
    });
  ...
  if (document.readyState === "loading") {
    document.addEventListener("DOMContentLoaded", initFinalPatch, {
      once: true,
    });
  } else {
    initFinalPatch();
...
// Cập nhật link icon person theo trạng thái đăng nhập
document.addEventListener("DOMContentLoaded", () => {
  const token = localStorage.getItem("kx_auth_token");
  const user = JSON.parse(localStorage.getItem("kx_user") || "{}");

  document.querySelectorAll('[data-auth-link]').forEach(el => {
    if (token) {
      el.href = "profile.html";
      // Hiện avatar chữ cái nếu có tên
